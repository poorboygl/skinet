...
function sheet_to_json_sna(sheet, opts){
	if(sheet == null || sheet["!ref"] == null) return [];
	var val = {t:'n',v:0}, header = 0, offset = 1, hdr = [], isempty = true, v=0, vv="";
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var o = opts != null ? opts : {};
	...
	for (R = r.s.r; R <= r.e.r; ++R) {
		...
		var returnObj = {};
		for (C = r.s.c; C <= r.e.c; ++C) {
			val = sheet[encode_col(C) + rr];
			v = val.v;
			returnObj[listField[C]] = v;
		}
		returnLst.push(returnObj);
	}
	return returnLst;
}

function sheet_to_json_model_info(sheet, opts, msg){
	if(sheet == null || sheet["!ref"] == null) return [];
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var listField = opts.listField;
	...
	var returnLst = [];
	r.e.r = lastRow(r,sheet);

	for (R = r.s.r; R <= r.e.r; ++R) {
		rr = encode_row(R);
		var returnObj = {};
		for (C = r.s.c; C <= r.e.c; ++C) {
			var val = sheet[encode_col(C) + rr];
			var v = '';
			if(val){
				v = val.v.toString().trim();
			}
			var validData =  validRowModelInfo(rr,listField[C],v.toUpperCase(), msg, opts.seasons);
	...
	var validSeason = find2Season(returnLst, r.s.r, msg);
	if(validSeason.status == 'FAIL'){
		return validSeason
	}

  //FEA #539 start 20193007 Add Model information
  var validMdlID = chkDupModelID(returnLst, r.s.r, msg, brand);
  if(validMdlID.status == 'FAIL'){
    return validMdlID
  }
  //FEA #539 end 20193007
...
function sheet_to_json_article_info(sheet, opts, msg){
	if(sheet == null || sheet["!ref"] == null) return [];
	var r = {s:{r:0,c:0},e:{r:0,c:0}};

	var listField = opts.listField;
	var listModelID = opts.modelListID;
	var range = opts.range;
	var brand = opts.brand;
	switch(typeof range) {
		case 'string': r = safe_decode_range(range); break;
	...
	for (R = r.s.r; R <= r.e.r; ++R) {
        ...
        var val = sheet[encode_col(C) + rr];
			...
			var gender = sheet[encode_col(18) + rr].v.toString().trim(); //get Gender to check valid modelID
			var validData = {};
			if (brand == 'ADIDAS SPORT PERFORMANCE' || brand == 'ADIDAS SPORTSWEAR' || brand == 'ADIDAS ORIGINALS'){
				var modelID = sheet[encode_col(12) + rr].v.toString().trim(); //get Model to check valid Season
				validData =  validRowArticleAdidasInfo(rr,listField[C],v.toUpperCase(), msg, listModelID, gender.toUpperCase(), opts.mdlDetails, modelID);
			}
			else{
        // Start 20190108 UFE #401 Wrong data when upload Article file
			// 	var modelID = sheet[encode_col(18) + rr].v.toString().trim(); //get Model to check valid Season
			// 	validData =  validRowArticleReebokInfo(rr,listField[C],v.toUpperCase(), msg, listModelID, opts.mdlDetails, modelID)
	...
	var validSeason = find2SeasonArticle(returnLst, r.s.r, msg);
	if(validSeason.status == 'FAIL'){
		return validSeason;
	}

	var validModelBrand = checkModelBrand(returnLst, r.s.r, msg, opts.mdlDetails, brand);
	if(validModelBrand.status == 'FAIL'){
		return validModelBrand
	}

	var validArtID = chkDupArtID(returnLst, r.s.r, msg, brand);
	if(validArtID.status == 'FAIL'){
		return validArtID;
	}
  ...
  function sheet_to_json_basic_info(sheet, opts, msg){
    if (sheet == null || sheet["!ref"] == null) return [];
    var r = {s: {r: 0, c: 0}, e: {r: 0, c: 0}};

    var listField = opts.listField;
    var range = opts.range;
    // var brand = sheet[encode_col(149) + rr].v.toString().trim();
    var brand;
    switch (typeof range) {
      case 'string':
    ...
    for (R = r.s.r; R <= r.e.r; ++R) {
      ...
      for (C = r.s.c; C <= r.e.c; ++C) {
        ...
        var validData;
        // var brand = sheet[encode_col(149) + rr].v.toString().trim();
        //TODO: start validation on excel data
        // var modelID = sheet[encode_col(5) + rr].v.toString().trim(); //get Model to check valid Season
        var validData = validRowModelOfBasicInfo(rr, listField[C], v.toUpperCase(), msg, opts.seasons);
        if (validData.status == 'FAIL') {
          return validData
        }
        brand = sheet[encode_col(148) + rr].v.toString().trim();
        if (brand != 'Reebok' && brand != 'Reebok Core' && brand != 'adidas Sport Performance' && brand != 'adidas Sportswear' && brand != 'adidas Originals') {
          validData.status = 'FAIL';
          validData.message = msg.BRAND_MSG.WRONG_BRAND;
          return validData;
        }
        else {
          var modelID = sheet[encode_col(8) + rr].v.toString().trim(); //get Model to check valid Season
          var gender = sheet[encode_col(26) + rr].v.toString().trim(); // get Gender  to check valid Season
          var outArray = [];
            validData = validRowArticleForBasicInfo(rr, listField[C], v.toUpperCase(), msg, opts.mdlDetails, modelID, brand, gender, outArray);
            if('fsProdMonth' == listField[C] || 'pullFwd' == listField[C]){
              v = outArray[0];
            }
        }
        if (validData.status == 'FAIL') {
  ...
  function sheet_to_json_issue_scope(sheet, opts, msg) {

    if(sheet == null || sheet["!ref"] == null) return [];

    //r : row; s: start; e: end; c: column
    var r = {s:{r:0,c:0},e:{r:0,c:0}};
    ...
    for (R = r.s.r; R <= r.e.r; ++R) {
      ...
      // loop start column of a row to end column of a row
      for (C = r.s.c; C <= r.e.c; ++C) {
        try {
          var val = sheet[encode_col(C) + rr];
        }catch(e){
        ...
        var validData = {};
        var materialType = sheet[encode_col(1) + rr].v.toString().trim();

        validData =  validRowIssueScope(rr,listField[C],v , msg, opts.issueScopeList, materialType);
    ...
    // var result = checkDuplicatedSolutionCode(returnLst, r.s.r, msg, opts.issueScopeList);
    // if (result.status == 'FAIL') {
    //   return result;
    // }

    result = checkDuplicatedIssueCode(returnLst, r.s.r, msg);
    if (result.status == 'FAIL') {
      return result;
    }
  ...
  function validRowIssueScope(row,field,value,msg, issueScopeList, materialType){
    var result = {
      status: 'SUCCESS',
      line: row
    };

    // if(field=='solutionCode'){
    //   if(value == ''){
    //     result.status = 'FAIL';
    //     result.message = msg.ISSUE_SCOPE_MSG.WR_EMPTY;
    //     return result;
    //   }
    //   //check duplicate
    // } else
    if(field=='materialType') {
      //materialtype only has 4 values
      if (value != '' && value != 'Leather' && value != 'Textile' && value != 'Synthetic' && value != 'Miscellaneous' ) {
        result.status = 'FAIL';
        result.message = msg.ISSUE_SCOPE_MSG.WR_MATERIAL_TYPE;
        return result;
      }
    } else if(field=='issueScopeCode'){
      ...
      //check if issue scopes is changing material type
      if(isChangedType(issueScopeList, value, materialType)){
        result.status = 'FAIL';
        result.message = msg.ISSUE_SCOPE_MSG.WR_ISSUE_SCOPE;
        return result;
      }
    }
    else if(field=='issueScopeName'){
      if(value == ''){
        result.status = 'FAIL';
        result.message = msg.ISSUE_SCOPE_MSG.WR_EMPTY;
        return result;
      }
    }else if(field=='warningMessageInSap'){
      if(value == ''){
        result.status = 'FAIL';
        result.message = msg.ISSUE_SCOPE_MSG.WR_EMPTY;
        return result;
      }
    }else if(field == 'level0' || field == 'level1'
      || field == 'level2' || field == 'level3'
      || field == 'level4') {

      if (value != 0 && value != 1) {
        result.status = 'FAIL';
        result.message = msg.ISSUE_SCOPE_MSG.WR_WARNING_LEVEL;
        return result;
      }
    }
    return result;
  ...
  function isChangedType(issueScopeList, issueScope, materialType) {
    for(let i = 0; i<issueScopeList.length; i++) {
      if(issueScope ==  issueScopeList[i].issueScopeCode
        && materialType != issueScopeList[i].materialType) {
        return true;
      }
    }
    return false;
  ...
  function chkValidTracking(seasonID,modelID,articleID,artIDLst,brandID) {

    let result = artIDLst.filter(item => item.brandID.toUpperCase() == brandID.toUpperCase()
      && item.season.toUpperCase() == seasonID.toUpperCase());

    for(let i = 0; i<result.length; i++) {
      if(seasonID.toUpperCase() ==  result[i].season.toUpperCase() && modelID.toUpperCase() == result[i].modelID.toUpperCase()
        && articleID.toUpperCase() ==  result[i].articleID.toUpperCase()) {
        return true;
      }
    }
    return false;
  ...
  function validRowTrackingInfo(row,field,value,msg, artIDLst,seasonID ,modelID,articleID,brandID){
    var result = {
      status: 'SUCCESS',
      line: row
    };
    ...
    if(field == 'brand_id') {
      if(brandID.toLowerCase() == 'reebok') {
        if(value.toLowerCase() != 'reebok' && value.toLowerCase() != 'reebok inline') {
          result.status = 'FAIL';
          result.message = msg.TRACKING_MSG.WR_BRAND_ID
          return result;
        }
      } else if (brandID.toLowerCase() != value.toLowerCase()) {
        result.status = 'FAIL';
        result.message = msg.TRACKING_MSG.WR_BRAND_ID;
        return result;
      }
    ...
    if(!chkValidTracking(seasonID ,modelID,articleID,artIDLst,brandID)) {
      result.status = 'FAIL';
      result.message = msg.TRACKING_MSG.WR_INVALID_INFO;
      return result;
    }

    if(field=='season_id'){

      let fStr = value.substr(0,2);
      let lStr = value.substr(2);
      if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 4){
        result.status = 'FAIL';
        result.message = msg.TRACKING_MSG.WR_SEASON;
        return result;
      }
    } else if(field=='model_id') {
      if (value.length != 5 ) {
        result.status = 'FAIL';
        result.message = msg.TRACKING_MSG.WR_MDL_CD;
        return result;
      }
    } else if(field=='gender'){
      if( value != '' && value != 'M' && value != 'K' && value != 'W' && value != 'U' && value != 'GS'
        && value != 'PS' && value != 'GSPS' && value != 'TD' && value != 'I' && value != 'C' && value != 'J' && value != 'UNI'){
        result.status = 'FAIL';
        result.message = msg.ARTICLE_MSG.WR_GENDER;
        return result;
      }
    }
      ...
      if(value.toString().length != 6){
        result.status = 'FAIL';
        result.message = msg.TRACKING_MSG.WR_ARTICLE;
        return result;
      }
    }
    // CS Tracking - Change template - TIEN - 2020/12/07 - MODIFY - START
    // else if(field == 'cs1_bottom_etc' || field == 'cs1_mold_etc_sel'
    //   || field == 'cs2_bottom_etc' || field == 'cs2_mold_etc_sel'
    //   || field == 'cs3_bottom_etc' || field == 'cs3_mold_etc_sel') {
    //   var isValid = true;
    //
    //   //Format MM/DD/YYYY
    ...
    //   if (!isValid) {
    //     result.status = 'FAIL';
    //     // result.message = (field == 'retailIntroDate') ? msg.ARTICLE_MSG.WR_RETAIL_INTRO_DATE : msg.ARTICLE_MSG.WR_RELEASE_DT;
    //     switch (field) {
    //       case 'cs1_bottom_etc':
    //         result.message = msg.TRACKING_MSG.WR_CS1_BOTTOM_ETC;
    //         break;
    //       case 'cs2_bottom_etc':
    //         result.message = msg.TRACKING_MSG.WR_CS2_BOTTOM_ETC;
    //         break;
    //       case 'cs3_bottom_etc':
    //         result.message = msg.TRACKING_MSG.WR_CS3_BOTTOM_ETC;
    //         break;
    //       case 'cs1_mold_etc_sel':
    //         result.message = msg.TRACKING_MSG.WR_CS1_MOLD_ETC;
    //         break;
    //       case 'cs2_mold_etc_sel':
    //         result.message = msg.TRACKING_MSG.WR_CS2_MOLD_ETC;
    //         break;
    //       case 'cs3_mold_etc_sel':
    //         result.message = msg.TRACKING_MSG.WR_CS3_MOLD_ETC;
    //         break;
    //       default:
    //         break;
    //     }
    ...
    //   }
    // }
    // else if(field =='pf_status' || field == 'cr2_arrive_fit_test' || field == 'cr2_arrive_wear_test'
    //   || field == 'cs1_meeting_eta' || field == 'cs1_meeting_schedule' || field == 'cs1_meeting_actual'
    //   || field == 'cs1_fit_sample_passing' || field == 'cs1_wear_sample_passing'
    //   || field == 'ta_issue' || field == 'cs2_meeting_due_date'
    //   || field == 'cs2_meeting_schedule' || field == 'cs2_meeting_actual' || field == 'cs2_fit_sample_passing'
    //   || field == 'cs3_meeting_due_date' || field == 'cs3_meeting_actual'
    //   || field == 'production_start' || field == 'production_crd'){
    //
    //   var isValid = true ;
    //
    //   //Format MM/DD/YYYY
    ...
    //     switch(field){
    //       case 'pf_status':
    //         result.message = msg.TRACKING_MSG.WR_PF_STATUS;
    //         break;
    //       case 'cr2_arrive_fit_test':
    //         result.message = msg.TRACKING_MSG.WR_CR2_ARRIVAL_FIT_TEST;
    //         break;
    //       case 'cr2_arrive_wear_test':
    //         result.message = msg.TRACKING_MSG.WR_CR2_ARRIVAL_WEAR_TEST;
    //         break;
    //       case 'cs1_meeting_eta':
    //         result.message = msg.TRACKING_MSG.WR_CS1_MEETING_ETA_DATE;
    //         break;
    //       case 'cs1_meeting_schedule':
    //         result.message = msg.TRACKING_MSG.WR_CS1_MEETING_SCHEDULE;
    //         break;
    //       case 'cs1_meeting_actual':
    //         result.message = msg.TRACKING_MSG.WR_CS1_MEETING_ACTUAL;
    //         break;
    //       case 'cs1_fit_sample_passing':
    //         result.message = msg.TRACKING_MSG.WR_CS1_FIT_SAMPLE_PASSING_FIT_TEST;
    //         break;
    //       case 'cs1_wear_sample_passing':
    //         result.message = msg.TRACKING_MSG.WR_CS1_WEAR_SAMPLE_PASSING_FIT_TEST;
    //         break;
    //       case 'ta_issue':
    //         result.message = msg.TRACKING_MSG.WR_TA_ISSUE;
    //         break;
    //       case 'cs2_meeting_due_date':
    //         result.message = msg.TRACKING_MSG.WR_MEETING_DUE_DATE;
    //         break;
    //       case 'cs2_meeting_schedule':
    //         result.message = msg.TRACKING_MSG.WR_MEETING_SCHEDULE;
    //         break;
    //       case 'cs2_meeting_actual':
    //         result.message = msg.TRACKING_MSG.WR_CS2_MEETING_ACTUAL;
    //         break;
    //       case 'cs2_fit_sample_passing':
    //         result.message = msg.TRACKING_MSG.WR_CS2_FIT_SAMPLE_PASSING_FIT_TEST;
    //         break;
    //       case 'cs3_meeting_due_date':
    //         result.message = msg.TRACKING_MSG.WR_CS3_MEETING_DUE_DATE;
    //         break;
    //       case 'cs3_meeting_actual':
    //         result.message = msg.TRACKING_MSG.WR_CS3_MEETING_ACTUAL;
    //         break;
    //       case 'production_start':
    //         result.message = msg.TRACKING_MSG.WR_START_PRODUCTION;
    //         break;
    //       case 'production_crd':
    //         result.message = msg.TRACKING_MSG.WR_START_PRODUCTION;
    //         break;
    ...
    //   }
    // }
    else if (field == 'cs1_meeting_result' || field == 'cs2_meeting_result' || field == 'cs3_meeting_result') {
      if(value == '' || value.contains('N/A')){
        return result;
      }
      if(!['pass', 'reject', 'holding'].includes(value.toLowerCase())) {
        result.status = 'FAIL';
        switch (field) {
          case 'cs1_meeting_result':
            result.message = msg.TRACKING_MSG.WR_CS1_MEETING_RESULT;
            break;
          case 'cs2_meeting_result':
            result.message = msg.TRACKING_MSG.WR_CS2_MEETING_RESULT;
            break;
          case 'cs3_meeting_result':
            result.message = msg.TRACKING_MSG.WR_CS3_MEETING_RESULT;
            break;
          default:
            break;
        }
    ...
    else if (field =='cs1_meeting_schedule' || field == 'cs1_meeting_actual'
      || field == 'cs1_fit_sample_sending_date' || field == 'cs1_wear_sample_sending_date'
      || field == 'tech_package_receive_date'
      || field == 'cs2_ta_deadline_date' || field == 'cs2_ta_actual_date' || field == 'cs2_meeting_schedule'
      || field == 'cs2_meeting_actual' || field == 'cs2_fit_sample_sending_date'
      || field == 'cs3_meeting_schedule' || field == 'cs3_meeting_actual'
      || field == 'production_cutting_die_order_date' || field == 'production_cutting_die_finish_date'|| field == 'production_send_laser_pallet_file'
      || field == 'production_send_pvc_jig_gauge' || field == 'production_1st_sdd') {
      var isValid = true ;
      var regexNum = /^[0-9]+$/;
      if(value == '' || value.contains('N/A') || !value){
        return result;
      }
      ...
      if(!isValid){
        ...
        switch(field){
          ...
          case 'cs1_meeting_actual':
            result.message = msg.TRACKING_MSG.WR_CS1_MEETING_ACTUAL;
            break;
          case 'cs1_fit_sample_sending_date':
            result.message = msg.TRACKING_MSG.WR_CS1_FIT_SAMPLE_SENDING_SEL;
            break;
          case 'cs1_wear_sample_sending_date':
            result.message = msg.TRACKING_MSG.WR_CS1_WEAR_SAMPLE_SENDING_SEL;
            break;
          case 'tech_package_receive_date':
            result.message = msg.TRACKING_MSG.WR_TECH_PACKAGE_RECEIVE_DATE;
            break;
          case 'cs2_ta_deadline_date':
            result.message = msg.TRACKING_MSG.WR_CS2_TA_DEADLINE_DATE;
            break;
          case 'cs2_ta_actual_date':
            result.message = msg.TRACKING_MSG.WR_CS2_TA_ACTUAL_DATE;
            break;
          case 'cs2_meeting_schedule':
            result.message = msg.TRACKING_MSG.WR_CS2_MEETING_SCHEDULE;
            break;
          case 'cs2_meeting_actual':
            result.message = msg.TRACKING_MSG.WR_CS2_MEETING_ACTUAL;
            break;
          case 'cs2_fit_sample_sending_date':
            result.message = msg.TRACKING_MSG.WR_CS2_FIT_SAMPLE_SENDING_SEL;
            break;
          case 'cs3_meeting_schedule':
            result.message = msg.TRACKING_MSG.WR_CS3_MEETING_SCHEDULE;
            break;
          case 'cs3_meeting_actual':
            result.message = msg.TRACKING_MSG.WR_CS3_MEETING_ACTUAL;
            break;
          case 'production_cutting_die_order_date':
            result.message = msg.TRACKING_MSG.WR_PRODUCTION_CUTTING_DIE_ORDER_DATE;
            break;
          case 'production_cutting_die_finish_date':
            result.message = msg.TRACKING_MSG.WR_PRODUCTION_CUTTING_DIE_FINISH_DATE;
            break;
          case 'production_send_laser_pallet_file':
            result.message = msg.TRACKING_MSG.WR_PRODUCTION_SEND_LASER_PALLET_FILE;
            break;
          case 'production_send_pvc_jig_gauge':
            result.message = msg.TRACKING_MSG.WR_PRODUCTION_SEND_PVC_JIG_GAUGE;
            break;
          case 'production_1st_sdd':
            result.message = msg.TRACKING_MSG.WR_PRODUCTION_1ST_SDD;
            break;
          default: break;
        }
        return result;
    ...
    else if (field == 'cs1_fit_sample_result_date' || field == 'cs1_wear_sample_result_date' || field == 'cs2_fit_sample_result_date') {
      var isValid = true ;
      //Format YYYY/MM/DD
      ...
      if(!(date_regex.test(value)) && !/Undertesting/i.test(value) && !/Under Testing/i.test(value) && !/skip/i.test(value))
      {
        isValid = false;
      }
      ...
      if(!isValid){
        result.status = 'FAIL';
        switch(field){
          case 'cs1_fit_sample_result_date':
            result.message = msg.TRACKING_MSG.WR_CS1_FIT_SAMPLE_RESULT_SEL;
            break;
          case 'cs1_wear_sample_result_date':
            result.message = msg.TRACKING_MSG.WR_CS1_WEAR_SAMPLE_RESULT_SEL;
            break;
          case 'cs2_fit_sample_result_date':
            result.message = msg.TRACKING_MSG.WR_CS2_FIT_SAMPLE_RESULT_SEL;
            break;
          default: break;
        }
        return result;
    ...
    else if(field == 'cs2_meeting_delay_reason' || field == 'cs3_meeting_delay_reason'){
      var isValid = true ;
      //Format YYYY/MM/DD
      var date_regex = /\d{4}[\/|-](0[1-9]|1[012])[\/|-](0[1-9]|[12][0-9]|3[01])/;
      ...
      if(!isValid){
        result.status = 'FAIL';
        switch(field){
          case 'cs2_meeting_delay_reason':
            result.message = msg.TRACKING_MSG.WR_CS2_MEETING_DELAY_REASON;
            break;
          case 'cs3_meeting_delay_reason':
            result.message = msg.TRACKING_MSG.WR_CS3_MEETING_DELAY_REASON;
            break;
          default: break;
        }
        return result;
      }
    } else if(field == 'tech_package_ctq_ctp') {
      var regexNum = /^[0-9]+$/;
      //Format YYYY/MM/DD
      // const date_regex = /\d{4}\/(0[1-9]|1[012])\/(0[1-9]|[12][0-9]|3[01])/;
      if(value == '' || value.contains('N/A') || value.toUpperCase() == 'NO') {
        return result;
      }
      if(!regexNum.test(value)) {
        result.status = 'FAIL';
        result.message = msg.TRACKING_MSG.WR_TECH_PACKAGE_CTQ_CTP;
        return result;
      }
    }
  ...
  //End 20190614 FEA #551

  function validRowTrackingInfo2(row, field, value, msg, seasonID ,brandID, checkLength) {
    var result = {
      status: 'SUCCESS',
      line: row
    };
    ...
    if (field =='cs1_meeting_schedule' || field == 'cs1_meeting_actual'
      || field == 'cs1_fit_sample_sending_date' || field == 'cs1_wear_sample_sending_date'
      || field == 'tech_package_receive_date'
      || field == 'cs2_ta_deadline_date' || field == 'cs2_ta_actual_date' || field == 'cs2_meeting_schedule'
      || field == 'cs2_meeting_actual' || field == 'cs2_fit_sample_sending_date'
      ...
      if(!(regexNum.test(value)))
      {
        if (!date_regex.test(value) || value.length !== 10) {
          isValid = false;
        }
      }
      if(!isValid){
    ...
    if (value && value.length > checkLength[field].size) {
      result.status = 'FAIL';
      result.message = checkLength[field].mess;
    }
    return result;
  }

  function sheet_to_json_csTracking_info(sheet, opts, msg) {
    ...
    var checkLength = {
      brand_id: {
        size: 50,
        mess: 'Wrong brand_id info. Maximum characters are 50.',
      },
      season_id: {
        size: 4,
        mess: 'Wrong season_id info. Maximum characters are 4.',
      },
      buy_month: {
        size: 50,
        mess: 'Wrong buy_month info. Maximum characters are 50.',
      },
      inline_ctm: {
        size: 50,
        mess: 'Wrong inline_ctm info. Maximum characters are 50.',
      },
      model_id: {
        size: 50,
        mess: 'Wrong model_id info. Maximum characters are 50.',
      },
      article_id: {
        size: 12,
        mess: 'Wrong article_id info. Maximum characters are 50.',
      },
      model_name: {
        size: 250,
        mess: 'Wrong model_name info. Maximum characters are 50.',
      },
      project_type: {
        size: 50,
        mess: 'Wrong project_type info. Maximum characters are 50.',
      },
      gender: {
        size: 20,
        mess: 'Wrong gender info. Maximum characters are 50.',
      },
      mold_id: {
        size: 50,
        mess: 'Wrong mold_id info. Maximum characters are 50.',
      },
      last_id: {
        size: 50,
        mess: 'Wrong last_id info. Maximum characters are 50.',
      },
      size_run: {
        size: 50,
        mess: 'Wrong size_run info. Maximum characters are 50.',
      },
      test_level: {
        size: 50,
        mess: 'Wrong test_level info. Maximum characters are 50.',
      },
      commo_plan: {
        size: 50,
        mess: 'Wrong commo_plan info. Maximum characters are 50.',
      },
      official_internal: {
        size: 50,
        mess: 'Wrong official_internal info. Maximum characters are 50.',
      },
      fty: {
        size: 50,
        mess: 'Wrong fty info. Maximum characters are 50.',
      },
      cs1_meeting_schedule: {
        size: 50,
        mess: 'Wrong cs1_meeting_schedule info. Maximum characters are 50.',
      },
      cs1_meeting_actual: {
        size: 50,
        mess: 'Wrong cs1_meeting_actual info. Maximum characters are 50.',
      },
      cs1_meeting_result: {
        size: 50,
        mess: 'Wrong cs1_meeting_result info. Maximum characters are 50.',
      },
      cs1_fit_sample_sending_date: {
        size: 50,
        mess: 'Wrong cs1_fit_sample_sending_date info. Maximum characters are 50.',
      },
      cs1_fit_sample_result_date: {
        size: 50,
        mess: 'Wrong cs1_fit_sample_result_date info. Maximum characters are 50.',
      },
      cs1_wear_sample_sending_date: {
        size: 50,
        mess: 'Wrong cs1_wear_sample_sending_date info. Maximum characters are 50.',
      },
      cs1_wear_sample_result_date: {
        size: 50,
        mess: 'Wrong cs1_wear_sample_result_date info. Maximum characters are 50.',
      },
      tech_package_receive_date: {
        size: 50,
        mess: 'Wrong tech_package_receive_date info. Maximum characters are 50.',
      },
      tech_package_ctq_ctp: {
        size: 50,
        mess: 'Wrong tech_package_ctq_ctp info. Maximum characters are 50.',
      },
      cs2_ta_deadline_date: {
        size: 50,
        mess: 'Wrong cs2_ta_deadline_date info. Maximum characters are 50.',
      },
      cs2_ta_actual_date: {
        size: 50,
        mess: 'Wrong cs2_ta_actual_date info. Maximum characters are 50.',
      },
      cs2_meeting_schedule: {
        size: 50,
        mess: 'Wrong cs2_meeting_schedule info. Maximum characters are 50.',
      },
      cs2_meeting_actual: {
        size: 50,
        mess: 'Wrong cs2_meeting_actual info. Maximum characters are 50.',
      },
      cs2_meeting_result: {
        size: 50,
        mess: 'Wrong cs2_meeting_result info. Maximum characters are 50.',
      },
      cs2_meeting_delay_reason: {
        size: 1000,
        mess: 'Wrong cs2_meeting_delay_reason info. Maximum characters are 1000.',
      },
      cs2_fit_sample_sending_date: {
        size: 50,
        mess: 'Wrong cs2_fit_sample_sending_date info. Maximum characters are 50.',
      },
      cs2_fit_sample_result_date: {
        size: 50,
        mess: 'Wrong cs2_fit_sample_result_date info. Maximum characters are 50.',
      },
      cs3_meeting_schedule: {
        size: 50,
        mess: 'Wrong cs3_meeting_schedule info. Maximum characters are 50.',
      },
      cs3_meeting_actual: {
        size: 50,
        mess: 'Wrong cs3_meeting_actual info. Maximum characters are 50.',
      },
      cs3_meeting_result: {
        size: 50,
        mess: 'Wrong cs3_meeting_result info. Maximum characters are 50.',
      },
      cs3_meeting_delay_reason: {
        size: 1000,
        mess: 'Wrong cs3_meeting_delay_reason info. Maximum characters are 1000.',
      },
      production_cwa_sign_off_date: {
        size: 50,
        mess: 'Wrong production_cwa_sign_off_date info. Maximum characters are 50.',
      },
      production_leadtime: {
        size: 50,
        mess: 'Wrong production_leadtime info. Maximum characters are 50.',
      },
      production_cutting_die_order_date: {
        size: 50,
        mess: 'Wrong production_cutting_die_order_date info. Maximum characters are 50.',
      },
      production_cutting_die_finish_date: {
        size: 50,
        mess: 'Wrong production_cutting_die_finish_date info. Maximum characters are 50.',
      },
      production_send_laser_pallet_file: {
        size: 50,
        mess: 'Wrong production_send_laser_pallet_file info. Maximum characters are 50.',
      },
      production_send_pvc_jig_gauge: {
        size: 50,
        mess: 'Wrong production_send_pvc_jig_gauge info. Maximum characters are 50.',
      },
      production_1st_sdd: {
        size: 50,
        mess: 'Wrong production_1st_sdd info. Maximum characters are 50.',
      },
      production_1st_sdd_prod_order_qty: {
        size: 50,
        mess: 'Wrong production_1st_sdd_prod_order_qty info. Maximum characters are 50.',
      },
      production_total_prod_order_qty: {
        size: 50,
        mess: 'Wrong production_total_prod_order_qty info. Maximum characters are 50.',
      },
    }

    //r : row; s: start; e: end; c: column
    var r = {s:{r:4,c:2},e:{r:0,c:0}};
    ...
    for (R = r.s.r; R <= r.e.r; ++R) {
      ...
      // loop start column of a row to end column of a row
      for (C = r.s.c; C <= r.e.c; ++C) {
        // CS Tracking - Change template - TIEN - 2020/12/07 - ADD - START
        // if([0, 4, 5, 10, 11, 16, 17, 22, 24, 25, 26, 27, 28, 29, 30, 38, 39, 40,
        //     45, 46, 47, 48, 49, 56, 57, 58, 59, 60, 61, 62, 63, 74, 78].includes(C)) {
        if([1, 5, 6, 11, 12, 17, 18, 23, 25, 26, 27, 28, 29, 30, 31, 39, 40, 41,
            46, 47, 48, 49, 50, 57, 58, 59, 60, 61, 62, 63, 64, 75, 79].includes(C)) {
          continue;
        }
        // CS Tracking - Change template - TIEN - 2020/12/07 - ADD - END
        try {
          var val = sheet[encode_col(C) + rr];
        }catch(e){
          console.log(e);
        ...
        var v = '';
        if(val && val.v && val.v.toString().trim().toLowerCase() !== 'n/a'){
          v = val.v.toString().trim();
          if ([32, 51, 65].includes(C) && v.toUpperCase() == 'TBD') {
            returnObj[listField[C]] = v;
            continue;
          }
        }
        if (val && val.v && val.v.toString().trim().toLowerCase() == 'n/a') {
          returnObj[listField[C]] = 'N/A';
          continue;
        }
        var validData = {};
        // CS Tracking - Change template - TIEN - 2020/12/07 - MODIFY
        var seasonID = sheet[encode_col(3) + rr].v.toString().trim();
        // var modelID = sheet[encode_col(8) + rr].v.toString().trim();
        // var articleID = sheet[encode_col(9) + rr].v.toString().trim();

        // if(brand == 'REEBOK CORE' ||  brand == 'REEBOK') {
        //   // rr : row; listField : columns in excel file, mdlDetails: columns in table TB_MODEL_LIST_M
        //   validData =  validRowTrackingInfo(rr,listField[C],v.toUpperCase(), msg,  opts.artIDLst,seasonID ,modelID,articleID,brand);
        // }else if (brand == 'ADIDAS RUNNING' || brand == 'ADIDAS CORE') {
        //   // CS Tracking - Change template - TIEN - 2020/12/07
        //   // var gender = sheet[encode_col(40) + rr].v.toString().trim(); //get Gender to check valid modelID
        //   var gender = sheet[encode_col(14) + rr].v.toString().trim(); //get Gender to check valid modelID
        //   validData =  validRowTrackingInfo(rr,listField[C],v.toUpperCase(), msg,  opts.artIDLst,seasonID ,modelID+gender,articleID,brand);
        // }
        validData =  validRowTrackingInfo2(rr, listField[C], v.toUpperCase(), msg, seasonID, brand, checkLength);

        if(validData.status == 'FAIL'){
          return validData
        };

        if([32, 33, 35, 37, 42, 44, 45, 51, 52, 55, 65, 66, 71, 72, 73, 74, 76, 69].includes(C) && v !== '' && v !== 'N/A' && typeof(val.v) == 'number') {
          var convertDate = new Date((v*1 - (25567 + 2))*86400*1000);
          v = convertDate.getFullYear().toString() + '/' + (convertDate.getMonth() + 1).toString().padStart(2, '0') + '/' + convertDate.getDate().toString().padStart(2, '0');
        }
        if(43 === C && v !== '' && v !== 'N/A' && v.toUpperCase() !== 'NO' && typeof(val.v) == 'number') {
          var convertDate2 = new Date((v*1 - (25567 + 2))*86400*1000);
          v = convertDate2.getFullYear().toString() + '/' + (convertDate2.getMonth() + 1).toString().padStart(2, '0') + '/' + convertDate2.getDate().toString().padStart(2, '0');
        }
        returnObj[listField[C]] = v;
      }
      returnLst.push(returnObj);
...
function sheet_to_json_fitwear_info(sheet, opts, msg){
	if(sheet == null || sheet["!ref"] == null) return [];
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var listField = opts.listField;
	var range = opts.range != null ? opts.range : sheet["!ref"];
	switch(typeof range) {
		case 'string': r = safe_decode_range(range); break;
		case 'number': r = safe_decode_range(sheet["!ref"]); r.s.r = range; break;
		default: r = range;
	...
	for (R = r.s.r; R <= r.e.r; ++R) {
		...
		for (C = r.s.c; C <= r.e.c; ++C) {
			...
			var validData =  validRowFitWearInfo(rr,listField[C],v.toUpperCase(), msg, opts.seasons, opts.articles, opts.stages);
			if(validData.status == 'FAIL'){
				return validData
			};
			returnObj[listField[C]] = v;
	...
	var validBrand = find2Brand(returnLst, r.s.r, msg);
	if(validBrand.status == 'FAIL'){
		return validBrand
	}

	var validArticleSeason = checkArticleSeason(returnLst, r.s.r, msg, opts.detailArticles);
	if(validArticleSeason.status == 'FAIL'){
		return validArticleSeason
	}
...
function sheet_to_json_fgt_info(sheet, opts, msg){
	if(sheet == null || sheet["!ref"] == null) return [];
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var listField = opts.listField;
	var range = opts.range != null ? opts.range : sheet["!ref"];
	...
	for (R = r.s.r; R <= r.e.r; ++R) {
		...
		for (C = r.s.c; C <= r.e.c; ++C) {
			var val = sheet[encode_col(C) + rr];
			var v = '';
      //Start 20190925 ERR #644 ERROR ON UPLOADING FGT
      //when the excel cell has the comment, will has an error. So need to add checking val.v
      if(val && val.v){
        //End 20190925 ERR #644
        v = val.v.toString().trim();
      }
      ...
      //end test
			...
			var validData =  validRowFgtInfo(rr, listField[C], v.toUpperCase(), msg, opts.seasons, opts.articles, opts.stages);
			if(validData.status == 'FAIL'){
				return validData
			};
			returnObj[listField[C]] = v;
	...
	var validArticleStage = find2ArticleStage(returnLst, r.s.r, msg);
	if(validArticleStage.status == 'FAIL'){
		return validArticleStage
	}
...
function sheet_to_json_msr_info(sheet, opts, msg){
	if(sheet == null || sheet["!ref"] == null) return [];
	var r = {s:{r:0,c:0},e:{r:0,c:0}};
	var listField = opts.listField;
	var range = opts.range != null ? opts.range : sheet["!ref"];
	...
	for (R = r.s.r; R <= r.e.r; ++R) {
		...
		for (C = r.s.c; C <= r.e.c; ++C) {
			...
			var validData =  validRowMSRInfo(rr,listField[C],v.toUpperCase(), msg, opts.seasons, opts.detailArticles);
			if(validData.status == 'FAIL'){
				return validData
			};
			returnObj[listField[C]] = v;
  ...
  function sheet_to_json_dr_claim(sheet, opts, msg) {
    if(sheet == null || sheet["!ref"] == null) return [];
    //r : row; s: start; e: end; c: column
    var r = {s:{r:1,c:0},e:{r:0,c:0}};
    ...
    for (R = r.s.r; R <= r.e.r; ++R) {
      ...
      for (C = r.s.c; C <= r.e.c; ++C) {
        var val = sheet[encode_col(C) + rr];
        if (listField[C] == 'production_date') {
          v = val ? val.w.toString().trim() : '';
        } else {
          v = val ? val.v.toString().trim() : '';
        }
        returnObj[listField[C]] = v;
      }
      returnLst.push(returnObj);
...
function lastRow(r,sheet){
	var lastRow = r.s.r;
	var notEmptyRow = true;
	while(notEmptyRow){
		notEmptyRow = false;
		var rr = encode_row(lastRow);
		for (col = r.s.c; col <= r.e.c; ++col) {
			var val = sheet[encode_col(col) + rr];
			if(val){
				notEmptyRow = true;
				break;
			}
...
function find2SeasonArticle(modelObjects, beginRow, msg){
	var result = {
			status: 'SUCCESS',
	};
	for(var i=0; i < modelObjects.length - 1; i++){
		var modelToChk = modelObjects[i];
		var toChkSeason = modelToChk.season;
		var num = 0;
		for(var j= i+1; j < modelObjects.length; j++){
			var modelChk = modelObjects[j];
			var checkingSeason = modelChk.season;
			if(toChkSeason != checkingSeason){
				return{
					status:'FAIL',
					message: msg.MODEL_MSG.TWO_SEASON,
					line: j + beginRow +1
				}
			}
		}
...
function find2Season(modelObjects, beginRow, msg){
	var result = {
			status: 'SUCCESS',
	};
	for(var i=0; i < modelObjects.length - 1; i++){
		var modelToChk = modelObjects[i];
		var toChkSeason = modelToChk.rpSeason;
		var num = 0;
		for(var j= i+1; j < modelObjects.length; j++){
			var modelChk = modelObjects[j];
			var checkingSeason = modelChk.rpSeason;
			if(toChkSeason != checkingSeason){
				return{
					status:'FAIL',
					message: msg.MODEL_MSG.TWO_SEASON,
...
function find2Brand(modelObjects, beginRow, msg){
	var result = {
			status: 'SUCCESS',
	};
	for(var i=0; i < modelObjects.length - 1; i++){
		var modelToChk = modelObjects[i];
		var toChkBrand = modelToChk.brand ?  modelToChk.brand: modelToChk.brandID;
		var num = 0;
		for(var j= i+1; j < modelObjects.length; j++){
			var modelChk = modelObjects[j];
			var checkingBrand = modelChk.brand ? modelChk.brand: modelChk.brandID;
			if(toChkBrand != checkingBrand){
				return{
					status:'FAIL',
					message: msg.MODEL_MSG.TWO_BRAND,
					line: j + beginRow +1
				}
			}
		}
...
function find2ArticleStage(modelObjects, beginRow, msg){
	var result = {
			status: 'SUCCESS',
	};
	for(var i=0; i < modelObjects.length - 1; i++){
		...
		for(var j= i+1; j < modelObjects.length; j++){
			var modelChk = modelObjects[j];
      //Start 20190916 ERR #637 FGT UPLOADING ERROR
      // 	if(modelToChk.article == modelChk.article && modelToChk.stage == modelChk.stage ){
      if(modelToChk.article == modelChk.article && modelToChk.stage == modelChk.stage && modelToChk.labRefNo == modelChk.labRefNo){
        //End 20190916 ERR #637
				return{
					status:'FAIL',
					message: msg.FGT_MSG.TWO_ART_STAGE,
					line: j + beginRow +1
				}
			}
		}
  ...
  function chkDupModelID(modelObjects, beginRow, msg, brand){
    var result = {
      status: 'SUCCESS',
    };
    for(var i=0; i < modelObjects.length - 1; i++){
      ...
      for(var j= i+1; j < modelObjects.length; j++){
        var modelChk = modelObjects[j];
        if(brand == 'ADIDAS SPORTSWEAR' || brand == 'ADIDAS SPORT PERFORMANCE'  || brand == 'ADIDAS ORIGINALS'){
          //FEA #539 start 20193007 Add Model information
          if(modelToChk.modelID == modelChk.modelID && modelToChk.mwID == modelChk.mwID && modelToChk.gender == modelChk.gender){
          //FEA #539 end 20193007
            return{
              status:'FAIL',
              message: msg.MODEL_MSG.DUP_MW_CD,
              line: j + beginRow +1
            }
          }
        }
          ...
          //FEA #539 start 20193007 Add Model information
          if(modelToChk.modelID == modelChk.modelID && modelToChk.mwID == modelChk.mwID){
          //FEA #539 end 20193007
            return{
              status:'FAIL',
              message: msg.MODEL_MSG.DUP_MW_CD,
...
function chkDupArtID(artObjects, beginRow, msg, brand){
	var result = {
			status: 'SUCCESS',
	};
	for(var i=0; i < artObjects.length - 1; i++){
		var artToChk = artObjects[i];
		var num = 0;
		for(var j= i+1; j < artObjects.length; j++){
			var artChk = artObjects[j];
			if(brand == 'ADIDAS SPORTSWEAR' || brand == 'ADIDAS SPORT PERFORMANCE' || brand == 'ADIDAS ORIGINALS'){
				if(artToChk.articleID == artChk.articleID){
					return{
						status:'FAIL',
						message: msg.ARTICLE_MSG.WR_DUP_ARTICLE,
						line: j + beginRow +1
					}
				}
			}
  ...
  function checkDuplicatedSolutionCode(artObjects, beginRow, msg, issueScopeList){
    var result = {
      status: 'SUCCESS',
    };
    //check out the current sheet
    for(var i=0; i < artObjects.length - 1; i++){
      var artToChk = artObjects[i];
      var num = 0;
      for(var j= i+1; j < artObjects.length; j++){
        var artChk = artObjects[j];
          if(artToChk.solutionCode == artChk.solutionCode){
            return{
              status:'FAIL',
              message: msg.ISSUE_SCOPE_MSG.WR_DUPLICATED_SOLUTION,
              line: j + beginRow +1
            }
          }
      }
    }
    //check out with current records
    for(var i=0; i < artObjects.length - 1; i++){
      var artToChk = artObjects[i];
      var num = 0;
      for(var j= 0; j < issueScopeList.length; j++){
        var artChk = issueScopeList[j];
        if(artToChk.solutionCode == artChk.solutionCode && artToChk.issueScopeCode != artChk.issueScopeCode){
          return{
            status:'FAIL',
            message: msg.ISSUE_SCOPE_MSG.WR_DUPLICATED_SOLUTION,
            line: i + beginRow + 2
  ...
  function checkDuplicatedIssueCode(artObjects, beginRow, msg){
    var result = {
      status: 'SUCCESS',
    };
    for(var i=0; i < artObjects.length - 1; i++){
      ...
      for(var j= i+1; j < artObjects.length; j++){
        var artChk = artObjects[j];
        if(artToChk.issueScopeCode == artChk.issueScopeCode){
          return{
            status:'FAIL',
            message: msg.ISSUE_SCOPE_MSG.WR_DUPLICATED_ISSUE_SCOPE,
            line: j + beginRow +1
          }
        }
      }
...
function checkModelBrand(modelObjects, beginRow, msg, modelLstDB, brand){
	var result = {
			status: 'SUCCESS',
	};
	for(var i=0; i < modelObjects.length; i++){
		var modelToChk = modelObjects[i];
		if(!isExistModelBrand(modelToChk, modelLstDB, brand)){
			return{
				status:'FAIL',
				message: msg.ARTICLE_MSG.WR_MODEL_BRAND,
				line: i + beginRow + 1
			}
		}
	}
...
function checkArticleSeason(modelObjects, beginRow, msg, articles){
	var result = {
			status: 'SUCCESS',
	};
	for(var i=0; i < modelObjects.length; i++){
		var modelToChk = modelObjects[i];
		if(!isExistArticleSeason(modelToChk,articles)){
			return{
				status:'FAIL',
				message: msg.FITWEAR_MSG.WR_ARTICLE_SEASON,
				line: i + beginRow + 1
			}
		}
	}
...
function isExistModelBrand(modelToChk, modelLst, brand){
  for(var i= 0; i < modelLst.length; i++){
    var modelChk = modelLst[i];
    if(brand == 'ADIDAS SPORTSWEAR' || brand == 'ADIDAS SPORT PERFORMANCE' || brand == 'ADIDAS ORIGINALS'){
      if( (modelToChk.modelID + modelToChk.gender) == modelChk.modelID && modelChk.brandID.toUpperCase() == brand){
        return true;
      }
    }
    else{
      // Start 20190108 UFE #401 Wrong data when upload Article file
      // if(modelToChk.mdlID == modelChk.modelID && modelChk.brandID.toUpperCase() == brand){
      if(modelToChk.modelID == modelChk.modelID && modelChk.brandID.toUpperCase() == brand){
        // End 20190108 UFE #401
        return true;
      }
    }
  }
...
function isExistArticleSeason(modelToChk,articles){
	for(var i= 0; i < articles.length; i++){
		var modelChk = articles[i];
		var season = modelToChk.season ? modelToChk.season : modelToChk.seasonID;
		var article = modelToChk.article ? modelToChk.article : modelToChk.articleID;
		if(season == modelChk.season && article == modelChk.articleID){
			return true;
		}
	}
	return false;
...
function validRowModelInfo(row, field, value, msg, seasons) {
  ...
  //Add 20181214: check the sports category column is empty
  // if (field == 'sportCgt') {
  //
  //   if(value == '') {
  //
  //     result.status = 'FAIL';
  //     result.message = msg.MODEL_MSG.WR_MDL_CD;
  //     return result;
  //   }
  // }
  //End add 20181214:
	...
	if(field=='modelSts'){
		if(value != 'RELEASED' && value != 'DROPPED' && value != 'UNDER CHANGE' && value != 'ACTIVATED' && value != 'ACTIVE' && value != 'IN-ACTIVATED' && value != 'IN-ACTIVE'
			&& value != 'PRE-ACTIVATED' && value != 'ON CHANGE - FROZEN' && value != 'BOM-FROZEN' && value != 'CREATED'){
			result.status = 'FAIL';
			result.message = msg.MODEL_MSG.WR_MDL_STS;
			return result;
		}
	}
  if(field=='modelID'){
    let fStr = value.substr(0,3);
    let lStr = value.substr(3);
    //let test = /^\d+$/.test(value);
    //AtMan | Start | UFE #679 Changing the valid modelID with 5 digits
    if(/^\d+$/.test(value) && value.length == 5){
      //do nothing. it's valid
    }
    //AtMan | End | UFE #679 Changing the valid modelID with 5 digits
    else if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 5){
      fStr = value.substr(0,2);
      lStr = value.substr(2);
      if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 5){

        //Start 20190814 ERR #614 UPLOADING ERROR ON MODEL BASIC INFO
        fStr = value.substr(0,1);
        lStr = value.substr(1);
        if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 5) {
          //End 20190814 ERR #614
          result.status = 'FAIL';
          result.message = msg.MODEL_MSG.WR_MDL_CD;
          return result;
        }
	...
	if(field=='gender'){
		if(value != 'M' && value != 'K' && value != 'W' && value != 'U' && value != 'GS' && value != 'PS' && value != 'GSPS' && value != 'TD' && value != 'I' && value != 'C' && value != 'J'){
			result.status = 'FAIL';
			result.message = msg.MODEL_MSG.WR_GENDER;
			return result;
		}
	}
	if(field=='rpSeason'){
		let fStr = value.substr(0,2);
		let lStr = value.substr(2);
		if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 4 || !isObjectExist(value,seasons)){
			result.status = 'FAIL';
			result.message = msg.MODEL_MSG.WR_SEASON;
			return result;
		}
  ...
  function validRowModelOfBasicInfo(row, field, value, msg, seasons) {
...
function isObjectExist(item, list){
	let rs = false;
	list.forEach(function(e){
		if(e.code.toUpperCase() == item){
			rs = true;
		}
	})
	return rs;
...
function isStringExist(item, list){
	let rs = false;
	list.forEach(function(e){
		if(e == item){
			rs = true;
...
function validRowFitWearInfo(row, field, value, msg, seasons, articles, stages){
	var result = {
			status: 'SUCCESS',
			line: row
	};
	if(field=='dept'){
      // Start 20190422 FEA #530 DEVELOPMENT CENTER CHANGE NAME
		if(value != '' && value != 'ADIDAS SPORTSWEAR' && value != 'ADIDAS SPORT PERFORMANCE'  && value != 'ADIDAS ORIGINALS'
            && value != 'REEBOK' && value != 'REEBOK ROYAL' && value != 'REEBOK CORE'){
      // End 20190422 FEA #530
			result.status = 'FAIL';
			result.message = msg.FITWEAR_MSG.WR_DEPT;
			return result;
		}
	}
		...
		if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 4 || !isObjectExist(value, seasons)){
			result.status = 'FAIL';
			result.message = msg.FITWEAR_MSG.WR_SEASON;
			return result;
		}

	}
	else if(field=='specialUsage') {
	  if (value != '' && value != 'IN LINE' && value != 'CTM' && value != 'SMU' && value != 'COMMO' && value != 'TEST') {
      result.status = 'FAIL';
      result.message = msg.FITWEAR_MSG.WR_SPECIAL_USAGE;
      return result;
    }
  }
	else if(field=='stage'){

		// if(value != '' && value != 'CR1' && value != 'CR2' && value != 'SMS/MCS' && value != 'CS1' && value != 'CS2'
		// 		&& value != 'CS3' && value != 'PROD' && value != 'SMU/CTM' && value != 'DR' && value != 'CLAIM'){
    if(value != '' && value != 'CR0' && value != 'CR1' && value != 'CR2' && value != 'PRESELL' && value != 'SMS'
      && value != 'MCS' && value != 'LOOK SEE' && value != 'CS1' && value != 'CS2' && value != 'CS3' && value != 'TEST'){
			result.status = 'FAIL';
			result.message = msg.FITWEAR_MSG.WR_STAGE;
			return result;
		}
		// if(!isObjectExist(value, stages)){
		// 	result.status = 'FAIL';
		// 	result.message = msg.FITWEAR_MSG.WR_STAGE_MAP;
		// 	return result;
		// }
	}
		...
		if(value.toString().length != 6){
			result.status = 'FAIL';
			result.message = msg.FITWEAR_MSG.WR_ARTICLE;
			return result;
		}
		if(!isObjectExist(value, articles)){
			result.status = 'FAIL';
			result.message = msg.FITWEAR_MSG.WR_ARTICLE_MAP;
			return result;
		}
	}
	else if(field=='fitStart' || field=='fitDeadlineShp' || field=='fitFinish' || field=='wearStart' || field=='wearDeadlineShp' || field=='wearFinish' ){
		var isValid = true ;
		//Format MM/DD/YYYY
    var date_regex = /^([1-9]|0[1-9]|1[0-2])\/([1-9]|0[1-9]|1\d|2\d|3[01])\/(19|20)\d{2}$/;
	    ...
	    if(!isValid){
	    	...
	    	switch(field){
	    		case 'fitStart':
	    				result.message =  msg.FITWEAR_MSG.WR_FIT_START;
	    				break;
	    		case 'fitDeadlineShp':
    				result.message =  msg.FITWEAR_MSG.WR_FIT_DEADLINE_SHP;
    				break;
	    		case 'fitFinish':
    				result.message =  msg.FITWEAR_MSG.WR_FIT_FINISH;
    				break;
	    		case 'wearStart':
    				result.message =  msg.FITWEAR_MSG.WR_WEAR_START;
    				break;
	    		case 'wearDeadlineShp':
    				result.message =  msg.FITWEAR_MSG.WR_WEAR_DEADLINE_SHP;
    				break;
	    		case 'wearFinish':
    				result.message =  msg.FITWEAR_MSG.WR_WEAR_FINISH;
    				break;
    			default: break;
...
function validRowFgtInfo(row, field, value, msg, seasons, articles, stages){
	var result = {
			status: 'SUCCESS',
			line: row
	};
	if(field=='brand'){
    // Start 20190418 FEA #535 Checking the attached FGT file
		if(value != '' && value.toUpperCase() != 'ADIDAS SPORTSWEAR' && value.toUpperCase() != 'ADIDAS SPORT PERFORMANCE' && value.toUpperCase() != 'ADIDAS ORIGINALS'
      && value.toUpperCase() != 'REEBOK' && value.toUpperCase() != 'REEBOK ROYAL' && value.toUpperCase() != 'REEBOK CORE'){
      // End 20190418 FEA #535
			result.status = 'FAIL';
			result.message = msg.FGT_MSG.WR_DEPT;
			return result;
		}
	}
	else if(field=='season'){
		...
		if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 4 || !isObjectExist(value, seasons)){
			result.status = 'FAIL';
			result.message = msg.FGT_MSG.WR_SEASON;
			return result;
		}
    ...
    if (value != '' && value != 'IN LINE' && value != 'CTM' && value != 'SMU' && value != 'COMMO' && value != 'TEST') {
      result.status = 'FAIL';
      result.message = msg.FGT_MSG.WR_SPECIAL_USAGE;
      return result;
    }
  }
	else if(field=='stage'){
    ...
    if(value != '' && value != 'CR0' && value != 'CR1' && value != 'CR2' && value != 'PRESELL' && value != 'SMS'
      && value != 'MCS' && value != 'LOOK SEE' && value != 'CS1' && value != 'CS2' && value != 'CS3' && value != 'TEST'){
			result.status = 'FAIL';
			result.message = msg.FGT_MSG.WR_STAGE;
			return result;
		}
		// if(!isObjectExist(value, stages)){
		// 	result.status = 'FAIL';
		// 	result.message = msg.FGT_MSG.WR_STAGE_MAP;
		// 	return result;
		// }
	}
	else if(field=='article'){
		if(value.toString().length != 6){
			result.status = 'FAIL';
			result.message = msg.FGT_MSG.WR_ARTICLE;
			return result;
		}
		if(!isObjectExist(value, articles)){
			result.status = 'FAIL';
			result.message = msg.FGT_MSG.WR_ARTICLE_MAP;
			return result;
		}
	}
	else if(field=='finalTestRslt' || field=='fgtRslt' || field=='ct09Rslt'){
		let validVals = ['', 'RELEASED','RELEASED-CHANGE MATERIAL','RELEASED-DROPPED','UNDERTESTING','UNDERTESTING-DROPPED','UNDERTESTING-MISSING','UNDERTESTING-CHANGE MATERIAL', 'DROPPED','REJECTED','REJECTED-DROPPED','REJECTED-MISSING','REJECTED-CHANGE MATERIAL'];
		if(!isStringExist(value, validVals)){
			result.status = 'FAIL';
			switch(field){
	    		case 'finalTestRslt':
					result.message =  msg.FGT_MSG.WR_FINAL_TEST_RSLT;
					break;
	    		case 'fgtRslt':
					result.message =  msg.FGT_MSG.WR_FGT_RSLT;
					break;
	    		case 'ct09Rslt':
					result.message =  msg.FGT_MSG.WR_CT09_RSLT;
					break;
	    		default: break;
			}
			return result;
  ...
  // Start ERR #688 DUPLICATE AND TRIPLICATE ENTRY
  // else if(field=='fgtFinishDt'){
  //   if(value == ''){
  //     result.status = 'FAIL';
  //     result.message =  msg.FGT_MSG.WR_FGT_FINISH_DT_EMPTY;
  //     return result;
  //   }
  //   var date_regex = /^([1-9]|0[1-9]|1[0-2])\/([1-9]|0[1-9]|1\d|2\d|3[01])\/(19|20)\d{2}$/;
  //   if(!(date_regex.test(value))){
  //     result.status = 'FAIL';
  //     result.message =  msg.FGT_MSG.WR_FGT_FINISH_DT_FORMAT;
  //     return result;
  //   }
  //   try {
  //     Date.parse(value);
  ...
  //   }
  // }
  // End ERR #688 DUPLICATE AND TRIPLICATE ENTRY
  else if(field=='fgtRevDt' || field=='fgtFinishDt' || field=='ctRevDt' || field=='ctFinishDt' || field=='fgtEtcFinish' || field=='ctEtcFinish'){
		var isValid = true ;
    // Start 20190702 UFE #555 Date unification
    // Start 20190418 FEA #535 Checking the attached FGT file
		//format MM/dd/yyyy
		var date_regex = /^([1-9]|0[1-9]|1[0-2])\/([1-9]|0[1-9]|1\d|2\d|3[01])\/(19|20)\d{2}$/;
    ...
    // End 20190702 UFE #555

    // End 20190418 FEA #535
		if(value == ''){
			return result;
		}
		var dateCheck = moment(value, 'MM/DD/YYYY').format('MM/DD/YYYY');
    if (dateCheck == 'Invalid date')
    {
      isValid = false;
	    ...
	    if(!isValid){
	    	...
	    	switch(field){
	    		case 'fgtRevDt':
    				result.message =  msg.FGT_MSG.WR_FGT_REV_DT;
    				break;
	    		case 'fgtFinishDt':
    				result.message =  msg.FGT_MSG.WR_FGT_FINISH_DT;
    				break;
	    		case 'ctRevDt':
    				result.message =  msg.FGT_MSG.WR_CT_REV_DT;
    				break;
	    		case 'ctFinishDt':
    				result.message =  msg.FGT_MSG.WR_CT_FINISH_DT;
    				break;
	    		case 'fgtEtcFinish':
    				result.message =  msg.FGT_MSG.WR_FGT_ETC_FINISH;
    				break;
	    		case 'ctEtcFinish':
    				result.message =  msg.FGT_MSG.WR_CT_ETC_FINISH;
    				break;
    			default: break;
	    	}
			return result;
  ...
  else if(field=='singMultiLay'){
		if(value != '' && value != 'S' && value != 'M'){
			result.status = 'FAIL';
			result.message =  msg.FGT_MSG.WR_SINGLE_MUL_LAY;
			return result;
		}
	}
	else if(field=='ct05Result'){
		if(value != '' && value.toUpperCase() != 'RELEASED' && value.toUpperCase() != 'REJECTED' && value.toUpperCase() != 'UNDERTESTING' && value.toUpperCase() != 'MISSING' && value.toUpperCase() != 'NO SUBMIT'){
			result.status = 'FAIL';
			result.message = msg.FGT_MSG.WR_CT05_RESULT;
			return result;
		}
	}
	else if(field == 'ct05RejReason'){
		let validVals = ['', 'FT-02','FT-07','FT-08','FT-11','GE-08','GE-09','GE-16','GE-27','GE-29','GE-38','GE-82','FGT-14'];
		if(!isStringExist(value, validVals)){
			result.status = 'FAIL';
			result.message =  msg.FGT_MSG.WR_CT05_REJ_REASON;
			return result;
		}
	}
	else if(field=='fgt01' || field=='fgt02' || field=='fgt04' || field=='fgt08' || field=='fgt13' || field=='fgt14' || field=='fgt1413' || field=='fgt22' || field=='fgt28' || field=='fgt30' || field=='fgt64'
		 	|| field=='fgt68' || field=='fgt75' || field=='fgt78' || field=='fgt012WksRef' || field=='fgt013WksRef' || field=='fgt0210C' || field=='fgt02After01' || field=='fgt02After14' || field=='fgt14FusEff'
		 	|| field=='ft07DryAg' || field=='ft10Color' || field=='ft14AgWash' || field=='st02Tear' || field=='st03Ten' || field=='st05Seam' || field=='ge24Flex'
		 	|| field=='ge29AbrCan' || field=='ge29AbrCot' || field=='ge63MartSan' || field=='ge63MartLine' || field=='ge48Adh' || field=='ft02Mig' || field=='ft08Cour' || field=='ge08Hyd'){
		if(value != '' && value != 'P' && value != 'F' && value != 'U'){
			result.status = 'FAIL';
	    	switch(field){
	    		case 'fgt01':
    				result.message =  msg.FGT_MSG.WR_FGT01;
    				break;
	    		case 'fgt02':
    				result.message =  msg.FGT_MSG.WR_FGT02;
    				break;
	    		case 'fgt04':
    				result.message =  msg.FGT_MSG.WR_FGT04;
    				break;
	    		case 'fgt08':
    				result.message =  msg.FGT_MSG.WR_FGT08;
    				break;
	    		case 'fgt13':
    				result.message =  msg.FGT_MSG.WR_FGT13;
    				break;
	    		case 'fgt14':
    				result.message =  msg.FGT_MSG.WR_FGT14;
    				break;
	    		case 'fgt1413':
    				result.message =  msg.FGT_MSG.WR_FGT1413;
    				break;
	    		case 'fgt22':
    				result.message =  msg.FGT_MSG.WR_FGT22;
    				break;
	    		case 'fgt28':
    				result.message =  msg.FGT_MSG.WR_FGT28;
    				break;
	    		case 'fgt30':
    				result.message =  msg.FGT_MSG.WR_FGT30;
    				break;
	    		case 'fgt64':
    				result.message =  msg.FGT_MSG.WR_FGT64;
    				break;
	    		case 'fgt68':
    				result.message =  msg.FGT_MSG.WR_FGT68;
    				break;
	    		case 'fgt75':
    				result.message =  msg.FGT_MSG.WR_FGT75;
    				break;
	    		case 'fgt78':
    				result.message =  msg.FGT_MSG.WR_FGT78;
    				break;
	    		case 'fgt012WksRef':
    				result.message =  msg.FGT_MSG.WR_FGT012_WKSREF;
    				break;
	    		case 'fgt013WksRef':
    				result.message =  msg.FGT_MSG.WR_FGT013_WKSREF;
    				break;
	    		case 'fgt0210C':
    				result.message =  msg.FGT_MSG.WR_FGT0210C;
    				break;
	    		case 'fgt02After01':
    				result.message =  msg.FGT_MSG.WR_FGT02_AFTER01;
    				break;
	    		case 'fgt02After14':
    				result.message =  msg.FGT_MSG.WR_FGT02_AFTER14;
    				break;
	    		case 'fgt14FusEff':
    				result.message =  msg.FGT_MSG.WR_FGT14_FUS_EFF;
    				break;
	    		case 'ft07DryAg':
    				result.message =  msg.FGT_MSG.WR_FT07_DRYAG;
    				break;
	    		case 'ft10Color':
    				result.message =  msg.FGT_MSG.WR_FT10_COL;
    				break;
	    		case 'ft14AgWash':
    				result.message =  msg.FGT_MSG.WR_FT14_AG_WASH;
    				break;
	    		case 'st02Tear':
    				result.message =  msg.FGT_MSG.WR_ST02_TEAR;
    				break;
	    		case 'st03Ten':
    				result.message =  msg.FGT_MSG.WR_ST03_TEN;
    				break;
	    		case 'st05Seam':
    				result.message =  msg.FGT_MSG.WR_ST05_SEAM;
    				break;
	    		case 'ge24Flex':
    				result.message =  msg.FGT_MSG.WR_GE24_FLEX;
    				break;
	    		case 'ge29AbrCan':
    				result.message =  msg.FGT_MSG.WR_GE29_ABR_CAN;
    				break;
	    		case 'ge29AbrCot':
    				result.message =  msg.FGT_MSG.WR_GE29_ABR_COT;
    				break;
	    		case 'ge63MartSan':
    				result.message =  msg.FGT_MSG.WR_GE63_MART_SAN;
    				break;
	    		case 'ge63MartLine':
    				result.message =  msg.FGT_MSG.WR_GE63_MART_LINE;
    				break;
	    		case 'ge48Adh':
    				result.message =  msg.FGT_MSG.WR_GE48_ADH;
    				break;
	    		case 'ft02Mig':
    				result.message =  msg.FGT_MSG.WR_FT02_MIG;
    				break;
	    		case 'ft08Cour':
    				result.message =  msg.FGT_MSG.WR_FT08_COUR;
    				break;
	    		case 'ge08Hyd':
    				result.message =  msg.FGT_MSG.WR_GE08_HYD;
    				break;
    			default: break;
	    	}
			return result;
...
function validRowMSRInfo(row, field, value, msg, seasons, articles){
	var result = {
			status: 'SUCCESS',
			line: row
	};
	...
	if(field=='brandID'){
    // Start 20190422 FEA #530 DEVELOPMENT CENTER CHANGE NAME
		if(value != 'ADIDAS SPORTSWEAR' && value != 'ADIDAS SPORT PERFORMANCE' && value != 'ADIDAS ORIGINALS'
            && value != 'REEBOK' && value != 'REEBOK ROYAL' && value != 'REEBOK CORE'){
    // End 20190422 FEA #530
			result.status = 'FAIL';
			result.message = msg.FITWEAR_MSG.WR_DEPT;
    ...
    if(value != '' && value != 'CR0' && value != 'CR1' && value != 'CR2' && value != 'PRESELL' && value != 'SMS'
      ...
      return result;
		...
		if(!/^[+-]?\d+(\.\d+)?$/.test(value)){
			result.status = 'FAIL';
			result.message = msg.MSR_MSG.WR_PRICE;
			return result;
		}
	}
	else if(field=='requestDate' || field=='ramCreate' || field=='gkCallDate' || field=='materialETC' || field=='materialETA' || field=='lapTestResultDate' || field=='improvedMaterialETC' || field=='improvedMaterialETA' ){
		var isValid = true ;

    //Start 20190719 UFE #605 DATE FORMAT
    // YYYYMMDD
		// var date_regex = /^(19|20)\d{2}(0[1-9]|1[0-2])(0[1-9]|1\d|2\d|3[01])$/;
    //MM/DD/YYYY
    var date_regex = /^([1-9]|0[1-9]|1[0-2])\/([1-9]|0[1-9]|1\d|2\d|3[01])\/(19|20)\d{2}$/;
    //End 20190719 UFE #605
	    ...
	    if(!isValid){
	    	...
	    	switch(field){
	    		case 'requestDate':
    				result.message =  msg.MSR_MSG.WR_REQ_DATE;
    				break;
	    		case 'ramCreate':
    				result.message =  msg.MSR_MSG.WR_RAM_CREATE_DATE;
    				break;
	    		case 'gkCallDate':
    				result.message =  msg.MSR_MSG.WR_GK_CALL_DATE;
    				break;
	    		case 'materialETC':
    				result.message =  msg.MSR_MSG.WR_MAT_ETC;
    				break;
	    		case 'materialETA':
    				result.message =  msg.MSR_MSG.WR_MAT_ETA;
    				break;
	    		case 'lapTestResultDate':
    				result.message =  msg.MSR_MSG.WR_LAP_TEST_RESULT_DATE;
    				break;
	    		case 'improvedMaterialETC':
    				result.message =  msg.MSR_MSG.WR_IMPROVE_MAT_ETC;
    				break;
	    		case 'improvedMaterialETA':
    				result.message =  msg.MSR_MSG.WR_IMPROVE_MAT_ETA;
    				break;
    			default: break;
...
function validRowArticleAdidasInfo(row,field,value,msg,mdlIDs, gender, mdlDetails, modelID){
	var result = {
			status: 'SUCCESS',
			line: row
	};
	...
	if(field=='season'){
		...
		if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 4){
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_SEASON;
			return result;
		}
		if(!isMappingModelSeason(value, modelID + gender, mdlDetails)){
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_MAP_MODEL_SEASON;
			return result;
		}
    ...
    // Start 20190930 UFE #646 The model id value can be entire number characters
    // Add 20181112 : upload Article
    // if(!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 5 ) {
    //   let fStr = value.substr(0, 2);
    //   let lStr = value.substr(2);
    //
    //   if (!/^\d+$/.test(lStr) || !/^[a-zA-Z]+$/.test(fStr) || value.length != 5 || !isObjectExist(value + gender, mdlIDs)) {
    //     //End add 20181112
    if(value.length != 5) {
    //End 20190930 UFE #646
        result.status = 'FAIL';
        result.message = msg.ARTICLE_MSG.WR_MDL_CD;
        return result;
      }
  }
	else if(field=='gender'){
		if( value != '' && value != 'M' && value != 'K' && value != 'W' && value != 'U' && value != 'GS'
      && value != 'PS' && value != 'GSPS' && value != 'TD' && value != 'I' && value != 'C' && value != 'J'){
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_GENDER;
			return result;
		}
		...
		if(value.toString().length != 6){
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_ARTICLE;
			return result;
		}
	}
	else if(field=='pullFwd' || field=='fsProdMonth'){
		var isValid = true ;
		var monthArr = value.toString().match(/^(\d{2})\-(\d{2})$/);
		if(monthArr){
			frMonth = monthArr[1];
			toMonth = monthArr[2];
			if(/^[a-zA-Z]+$/.test(frMonth) || parseInt(frMonth) < 1 || parseInt(frMonth) > 12 || /^[a-zA-Z]+$/.test(toMonth) || parseInt(toMonth) < 1 || parseInt(toMonth) > 12){
				isValid = false;
			}
		}
		else{
		...
		if(!isValid){
			result.status = 'FAIL';
			result.message = (field=='pullFwd') ? msg.ARTICLE_MSG.WR_PULL_FWD: msg.ARTICLE_MSG.WR_FIRST_PRODUCTION_MONTH;
			return result;
		}
	}
  // Start 20190419 FEA #537 Uploading Form
	else if(field=='retailIntroDate'  || field=='releaseDt'){
    var isValid = true ;
		// var isValid = false ;
		//format yyyyMMdd
        // var dateArr = value.toString().match(/^(\d{4})(\d{2})(\d{2})$/);
        // if (dateArr) {
	     //    var yyyy = +dateArr[1], mm = +dateArr[2], dd = +dateArr[3];
	     //    var date = new Date(yyyy, mm - 1, dd);
	     //    isValid = (date.getFullYear() === yyyy && date.getMonth() === mm - 1) ;
        // }
  ...
  // End 20190419 FEA #537

	else if(field=='pbType'){
    //Start 20190927 UFE #645 Adding the PROMO COMMERCIAL value for the rp name field
    //Add 20181128: UFE #267 Could upload the Article data with the PB type value = 'EM' and 'NT' in the Add Article Info screen
		if(value != 'CO' && value != 'CU' && value != 'MU' && value != 'NB' && value != 'NU' && value.replace(/ /g,'') != 'NU,NB'  && value != 'NC'
			&& value != 'NM' && value != 'NS' && value != 'TD' && value != 'TN' && value != 'MC' && value != 'EM' && value != 'NT'
      && value != 'MS'){
      //Edd add 20181128 UFE #267
      //End 20190927 UFE #645
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_PB_TYPE;
			return result;
		}
	}
	else if(field=='prodFTY'){
    // Start 20190419 FEA #540 Add the values to the RP Name column and Prod.FTY column
		if(value != 'HSD' && value != 'HVA' && value != 'HWA' && value.replace(/[' ']/g,'') != 'HWA-HWI'
      && value != 'HWD' && value != 'HWI' && value != 'PCI' && value != 'PWI'
      && value != 'SMA' && value != 'SPG' && value != 'HWT'&& value != 'LYV' && value != 'SHW' && value != 'HDS' && value != 'HWR'){
      // End 20190419 FEA #540
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_PROD_FTY;
			return result;
		}
	}
	else if(field=='rpNm'){
    //Start 20190927 UFE #645 Adding the PROMO COMMERCIAL value for the rp name field
    // Start 20190419 FEA #540 Add the values to the RP Name column and Prod.FTY column
		if(value != 'INLINE' && value != 'INLINE RISK' && value != 'INLINE DROPPED' && value != 'INLINE TRANSFERRED'
        && value != 'INLINE LATE ADD' && value != 'COUNTRY SMU' && value != 'KEY ACCOUNT SMU'
        && value != 'INLINE TRANSFERRED DROPPED' && value != 'SMU TRANSFERRED' && value != 'SMUX TRANSFERRED'
      && value != 'SMU TRANSFERRED DROPPED' && value != 'SMUX' && value != 'SMU' && value != 'SMU DROPPED'
		  && value != 'CTM' && value != 'INLINE IM' && value != 'INLINE OR' && value != 'SMU OR' && value !='SMU IM'
      && value != 'PROMO COMMERCIAL' && value != 'FO/CLEARANCE PRODUCT' && value != 'PROMO NON-COMMERCIAL' && value != 'OWN RETAIL EXCLUSIVE')
    // End 20190419 FEA #540
    //End 20190927 UFE #645
		{
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_RP_NM;
			return result;
		}
	}
	else if(field=='testLevel'){
		if(value !='A' && value !='B' && value !='C' && value !='C-OC' && value !='D'
			&& value !='D-OC' && value !='E' && value !='E-OC' && value !='DNT' && value !='TBA'&& value !='F'&& value !='X'){
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_PB_TYPE;
			return result;
		}
    ...
    //Start 20191204 SUP #677 Add the Transferred value on the remark column when user upload the article file
		if(value !='ACTIVATED' && value !='CREATED' && value !='PRE-ACTIVATED'
      && value !='RELEASED' && value !='DROPPED' && value !='ON CHANGE - FROZEN'
      && value !='BOM-FROZEN' && value !='ON CHANGE' && value != 'TRANSFERRED'
      && value != 'ON HOLD' && value != 'UNDER CHANGE'){
      //End 20191204 SUP #677
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_REMARK;
			return result;
		}
	}
	else if(field=='patternID' || field=='toolID' || field=='outToolID' || field=='midToolID' || field=='mtrlID' || field=='forecast' || field=='lt9060'){
		if(!/^\d+$/.test(value)){
			result.status = 'FAIL';
			switch(field){
				case 'patternID':
					result.message = msg.ARTICLE_MSG.WR_PATTERN;
					break;
				case 'toolID':
					result.message = msg.ARTICLE_MSG.WR_TOOL;
					break;
				case 'outToolID':
					result.message = msg.ARTICLE_MSG.WR_OUT_TOOL;
					break;
				case 'midToolID':
					result.message = msg.ARTICLE_MSG.WR_MID_TOOL;
					break;
				case 'mtrlID':
					result.message = msg.ARTICLE_MSG.WR_MET_WAY;
					break;
				case 'forecast':
					result.message = msg.ARTICLE_MSG.WR_FORECAST;
					break;
				case 'lt9060':
					result.message = msg.ARTICLE_MSG.WR_LT;
					break;
				default: break;
			}
			return result;
...
function isMappingModelSeason(season, modelID, modelList){

	for(let i = 0; i < modelList.length; i++){

		if(modelID.toUpperCase() == modelList[i].modelID.toUpperCase() && season.toUpperCase() == modelList[i].rpSeason.toUpperCase()){
  ...
  // Start 20190108 UFE #401 Wrong data when upload Article file
  function validRowArticleReebokInfo(row,field,value,msg,mdlIDs, mdlDetails, modelID) {
	var result = {
			status: 'SUCCESS',
			line: row
	};
	...
	if(field=='modelID') {
    // Start 20190403 FEA 519 Can uploading the RBK- MKTDRVN , KEY ACCOUNT SMU value in Special Usage column
    // let fStr = value.substr(0, 3);
    // let lStr = value.substr(3);
    //
    // // Add 20181112 : upload Article
    ...
    if (value.length != 5 || !isObjectExist(value , mdlIDs)) {
      // End  20190403 FEA 519
		...
		if(!isMappingModelSeason(value, modelID, mdlDetails)){
			result.status = 'FAIL';
			result.message = msg.ARTICLE_MSG.WR_MAP_MODEL_SEASON;
			return result;
		}
	...
	// else if(field=='gender'){
	// 	if(value !='' && value != 'BOYS' && value != 'GIRLS' && value != 'MENS' && value != 'WOMENS' && value != 'UNISEX - ADULTS'){
	// 		result.status = 'FAIL';
	// 		result.message = msg.ARTICLE_MSG.WR_GENDER;
	// 		return result;
	// 	}
    ...
    if( value != '' && value != 'M' && value != 'K' && value != 'W' && value != 'U' && value != 'GS' && value != 'PS' && value != 'GSPS' && value != 'TD' && value != 'I' && value != 'C' && value != 'J'){
      result.status = 'FAIL';
      result.message = msg.ARTICLE_MSG.WR_GENDER;
      return result;
    }
  }
	// else if(field=='smplSize'){
	//   //check if no any number character
	// 	if(!/^\d+$/.test(value)){
	// 		result.status = 'FAIL';
	// 		result.message = msg.ARTICLE_MSG.WR_SMPL_SIZE;
	// 		return result;
	// 	}
	//  }
	else if(field=='pullFwd'){
		var isValid = true ;
		var monthArr = value.toString().match(/^(\d{2})\-(\d{2})$/);
		if(monthArr){
			frMonth = monthArr[1];
  ...
  // Start 20190403 FEA 519 Can uploading the RBK- MKTDRVN , KEY ACCOUNT SMU value in Special Usage column
    ...
    var date_regex = /^([1-9]|0[1-9]|1[0-2])\/([1-9]|0[1-9]|1\d|2\d|3[01])\/(19|20)\d{2}$/;
	    ...
	    if(!isValid){
	    	...
	    	switch(field){
				case 'cwaSignOffsche':
					result.message = msg.ARTICLE_MSG.WR_CWA_DEADLINE_OFF; //FEA 519
					break;
				default: break;
	    	}
			return result;
    ...
    // Start 20190419 FEA #537 Uploading Form
  else if(field=='bomFrozen' || field=='retailIntroDate'  || field=='releaseDt'){
    var isValid = true ;
    // var isValid = false ;
    // //check yyyyMMdd
    // var dateArr = value.toString().match(/^(\d{4})(\d{2})(\d{2})$/);
    ...
    if(!isValid){
      ...
      switch(field){
        case 'bomFrozen':
          result.message = msg.ARTICLE_MSG.WR_BOM_FZ;
          break;
        case 'retailIntroDate':
          result.message = msg.ARTICLE_MSG.WR_RETAIL_INTRO_DATE;
          break;
        case 'releaseDt':
          result.message = msg.ARTICLE_MSG.WR_RELEASE_DT;
          break;
        default: break;
      }
  ...
  // End 20190419 FEA #537
    ...
    // Start 20190426 FEA #552 Add the values to PB type info
    //Add 20181128: UFE #267 Could upload the Article data with the PB type value = 'EM' and 'NT' in the Add Article Info screen
    if(value != 'CO' && value != 'CU' && value != 'MU' && value != 'NB' && value != 'NU' && value.replace(/ /g,'') != 'NU,NB'  && value != 'NC'
      && value != 'NM' && value != 'NS' && value != 'TD' && value != 'TN' && value != 'MC'
      && value != 'EM' && value != 'NT' && value != 'NO' && value != 'MS' && value != 'EO' && value != 'MC')
    {
      //Edd add 20181128 UFE #267
      // End 20190426 FEA #552
      result.status = 'FAIL';
      result.message = msg.ARTICLE_MSG.WR_PB_TYPE;
      return result;
    }
    ...
    if(value != 'HSD' && value != 'HVA' && value != 'HWA' && value.replace(/[' ']/g,'') != 'HWA-HWI' && value != 'HWD' && value != 'HWI' && value != 'PCI' && value != 'PWI' && value != 'SMA' && value != 'SPG' && value != 'HWT'&& value != 'LYV' && value != 'HWR'){
      result.status = 'FAIL';
      result.message = msg.ARTICLE_MSG.WR_PROD_FTY;
      return result;
    }
    ...
    // Start 20200713 FEA #758 ADDITIONAL DATA 'B2B' ON COLUMN AC - SPECIAL USAGE
    //Start ERR #570
	  //Start 20190419 FEA #537 Uploading Form
    // Start 20190304 FEA 519 Can uploading the RBK- MKTDRVN , KEY ACCOUNT SMU value in Special Usage column
    if(value != 'INLINE' && value != 'INLINE RISK' && value != 'INLINE DROPPED'
      && value != 'INLINE TRANSFERRED' && value != 'INLINE TRANSFERRED DROPPED' && value != 'SMU TRANSFERRED'
      && value != 'SMUX TRANSFERRED' && value != 'SMU TRANSFERRED DROPPED'
      && value != 'SMUX' && value != 'SMU' && value != 'SMU DROPPED' &&  value != 'RBK- MKTDRVN'
      && value != 'KEY ACCOUNT SMU' && value != 'ASIA-DRIVEN KA' && value != 'PROMO COMMERCIAL'
      && value != 'COUNTRY SMU' && value != 'RBK- LIMITED EDITION' && value != 'PROMO NON-COMMERCIAL'
      && value != 'PROMO' && value !='NAM-DRIVEN INLINE' && value !='ASIA-DRIVEN INLINE' && value != 'PROMO GENERAL'
      && value != 'NAM-DRIVEN KA' && value != 'LIMITED EDITION' && value != 'CTM' && value != 'FO/CLEARANCE PRODUCT' && value != 'INLINE LATE ADD' && value != 'OWN RETAIL EXCLUSIVE'
      && value != 'B2B'){
      //End 20190304 FEA 519
      //End 20190419 FEA #537
      //End ERR #570
      //End 20200713 FEA #758
      result.status = 'FAIL';
      result.message = msg.ARTICLE_MSG.WR_RP_NM;
      return result;
    }
      ...
      if(value !='A' && value !='B' && value !='C' && value !='C-OC' && value !='D'
        && value !='D-OC' && value !='E' && value !='E-OC'
        && value !='DNT' && value !='TBA'&& value !='F'&& value !='X'){
...
// End 20190108
  ...
  function validRowArticleForBasicInfo(row, field, value, msg, mdlDetails, modelID, brand, gender, outArray) {
    var result = {
      status: 'SUCCESS',
      line: row
    };
    if (!value) {
    ...
    else if (field == 'articleID' && (brand == 'adidas Sport Performance' || brand == 'adidas Sportswear' || brand == 'adidas Originals')) {
      if (value.toString().length != 6) {
        result.status = 'FAIL';
        result.message = msg.ARTICLE_MSG.WR_ARTICLE;
        return result;
      ...
      var date_regex = /^([1-9]|0[1-9]|1[0-2])\/([1-9]|0[1-9]|1\d|2\d|3[01])\/(19|20)\d{2}$/;
      if(date_regex.test(value)){
        if (moment(value, 'MM/DD/YYYY').format('MM/DD/YYYY') == 'Invalid date')
        {
          isValid = false;
        }
        else{
          var monthArray = value.toString().match(date_regex);
          var temp = 2 + + monthArray[1];
          if(temp == '13' || temp == '14'){
            temp = temp == '13' ? '1' : '2';
          }
          temp = temp.toString().length < 2 ? '0' + temp : temp;
          value = monthArray[1] + '-' + temp;
        }
      }
      var monthArr = value.toString().match(/^(\d{2})\-(\d{2})$/);
    ...
    if (brand == 'Reebok' || brand == 'Reebok Core') {
      if (field == 'pbType') {
        // Start 20190426 FEA #552 Add the values to PB type info
        //Add 20181128: UFE #267 Could upload the Article data with the PB type value = 'EM' and 'NT' in the Add Article Info screen
        if (value != 'CO' && value != 'CU' && value != 'MU' && value != 'NB' && value != 'NU' && value.replace(/ /g, '') != 'NU,NB' && value != 'NC'
        ...
        if (value != 'HSD' && value != 'HVA' && value != 'HWA' && value.replace(/[' ']/g, '') != 'HWA-HWI' && value != 'HWD' && value != 'HWI' &&
          value != 'PCI' && value != 'PWI' && value != 'SMA' && value != 'SPG' && value != 'HWT' && value != 'LYV' && value != 'HWR') {
          result.status = 'FAIL';
          result.message = msg.ARTICLE_MSG.WR_PROD_FTY;
          return result;
        }
        ...
        if (value != 'INLINE' && value != 'INLINE RISK' && value != 'INLINE DROPPED'
          ...
          && value != 'COUNTRY SMU' && value != 'RBK- LIMITED EDITION' && value != 'PROMO NON-COMMERCIAL'
          && value != 'PROMO' && value != 'NAM-DRIVEN INLINE' && value != 'ASIA-DRIVEN INLINE' && value != 'PROMO GENERAL'
          && value != 'NAM-DRIVEN KA' && value != 'LIMITED EDITION' && value != 'CTM' && value != 'FO/CLEARANCE PRODUCT'
          && value != 'INLINE LATE ADD' && value != 'OWN RETAIL EXCLUSIVE'
          && value != 'B2B') {
          //End 20190304 FEA 519
          //End 20190419 FEA #537
          //End ERR #570
    ...
    else if(brand == 'adidas Sport Performance' || brand == 'adidas Sportswear' || brand == 'adidas Originals'){
     if (field == 'pbType') {
        if (value != 'CO' && value != 'CU' && value != 'MU' && value != 'NB' && value != 'NU' && value.replace(/ /g, '') != 'NU,NB' && value != 'NC'
          && value != 'NM' && value != 'NS' && value != 'TD' && value != 'TN' && value != 'MC' && value != 'EM' && value != 'NT'
          && value != 'MS') {
...
var utils = {
	...
	sheet_to_row_object_array_sna: sheet_to_json_sna,
	sheet_to_row_object_array_model_info: sheet_to_json_model_info,
	sheet_to_row_object_array_article_info: sheet_to_json_article_info,
  sheet_to_row_object_array_basic_info: sheet_to_json_basic_info,
	sheet_to_row_object_array_fitwear_info: sheet_to_json_fitwear_info,
	sheet_to_row_object_array_fgt_info: sheet_to_json_fgt_info,
	sheet_to_row_object_array_msr_info: sheet_to_json_msr_info,
  //Start 20190614 FEA #551 Add a new function about adding CS Tracking in the excel file
  sheet_to_row_object_array_tracking_info: sheet_to_json_csTracking_info,
  //End 20190614 FEA #551
  sheet_to_row_object_array_issue_scope: sheet_to_json_issue_scope,
  sheet_to_row_object_array_dr_claim: sheet_to_json_dr_claim
};
